...
class Model:
    ...
    def loadWeights(self):
        save_load_path = 'weights/weight_84_1.h5' #change the path
...
class preProcessor:
    ...
    def cropFaces(self,image):
        ...
        for (x1, y1, x2, y2) in face_locations:
            if flag:
                #self.frame_and_face_locations.append((image,face_locations))
                flag = False
            try:
    ...
    # def getFramesAndFaceLocations(self):
    #     return self.frame_and_face_locations
...
class predictor:
    ...
    def allowedVideo(self,filename):
        allowed_video = set(['mp4','gif','webm','avi','3gp','mkv'])
        if (filename.rsplit('.',1)[1].lower() in allowed_video):
            return False
        else: 
            return True
...
def predict_page(request):
    if request.method == "GET":
        ...
        # out = cv.VideoWriter('check.mp4',cv.VideoWriter_fourcc(*'DIVX'), fps, size)
        # for i in range(len(frames_images)):
        #     out.write(frames_images[i])
        # out.release()
        ...
        #     #if images_lists[i] not in new_image:
        #     if pred[i] == 0 and labels_list[i] == 1:
        #         image = cv.rectangle(copy.deepcopy(images_lists[i]), start_point, end_point, (255, 0, 0), thickness)
        #         cv.putText(image, 'Fake|Real', (4, 21), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        #         cv.imwrite('otherResults/image_'+str(i)+'.png', image)
        #     elif pred[i] == 1 and labels_list[i] == 0:
        #         image = cv.rectangle(copy.deepcopy(images_lists[i]), start_point, end_point, (255, 0, 0), thickness)
        #         cv.putText(image, 'Real|Fake', (4, 21), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        #         cv.imwrite('otherResults/image_'+str(i)+'.png', image)
